...
const FILTER_COPY = {
  'zh-CN': {
    ...
    stage: { all: '全部', building: '建设中', active: '活跃', maintenance: '维护中', research: '研究中', archived: '已归档' },
    source: { all: '全部', github: 'GitHub', local: '本地', private: '私有', hybrid: '混合' },
    type: { all: '全部', website: '站点', backend: '后端', tooling: '工具', infra: '基础设施', research: '研究', agent: '智能体', data: '数据', library: '库' },
  },
  en: {
    searchLabel: 'Search',
    ...
    stage: { all: 'All', building: 'Building', active: 'Active', maintenance: 'Maintenance', research: 'Research', archived: 'Archived' },
    source: { all: 'All', github: 'GitHub', local: 'Local', private: 'Private', hybrid: 'Hybrid' },
    type: { all: 'All', website: 'Website', backend: 'Backend', tooling: 'Tooling', infra: 'Infrastructure', research: 'Research', agent: 'Agent', data: 'Data', library: 'Library' },
  },
} as const

export function ProjectsCatalogFilters({ query, onChange, onReset, tagCloud, fetchStatus, resultsMeta, locale }: ProjectsCatalogFiltersProps) {
    ...
    <section className="control-strip" aria-label="Project controls">
      ...
      <div className="control-strip-meta" aria-label="Catalog filters">
        <label className="check-item featured-only-label">
          <input type="checkbox" id="featured-only" checked={query.featuredOnly} onChange={(event) => onChange({ featuredOnly: event.target.checked })} /> {copy.featuredOnly}
        </label>
        <div className="tag-cloud control-tag-cloud" data-tag-cloud>
          {tagCloud.length > 0
            ? tagCloud.slice(0, 12).map((tag) => (
                <span key={tag.value || tag.label} className="tag">
                  {tag.label}
                </span>
              ))
            : <span className="tag">{query.search || query.featuredOnly ? copy.noTags : copy.waitingTags}</span>}
        </div>
