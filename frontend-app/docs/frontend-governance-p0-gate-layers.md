...
| L2 | 壳层视觉稳定与关键区域存在 | `npm run gate:l2` | 壳层关键区域缺失；跨页 header 稳定性超阈值；未归因整页差异 |
...
## 3. baseline-site 角色冻结
1. `baseline-site` 仅作为迁移期历史参照。
2. 旧站整页像素对比结果仅用于诊断，不再单独决定放行。
3. L2 的放行核心是壳层关键区域存在性与跨页稳定性。
...
2. 无归因项禁止进入白名单。
3. `suspected-regression` 不允许长期滞留，必须转缺陷或修复任务。
